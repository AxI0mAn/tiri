...
<script>
	...
	async function handleSave() {
		...
		try {
			...
			goto(`${base}/day`);
			/*  заменить goBack() на goto(\${base}/day`)`.
				Это гарантирует, что после сохранения заметки (созданной из напоминания) 
				страница всегда закрывается с первого раза на всех устройствах.
			*/
		} catch (err) {
